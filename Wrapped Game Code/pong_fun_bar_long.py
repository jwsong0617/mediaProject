...
class GameState:
    ...
    def frame_step(self,input_vect):
        ...
        if self.circle_x <= self.bar1_x + 10.:
            if self.circle_y >= self.bar1_y - 15 and self.circle_y <= self.bar1_y + 85:
                self.circle_x = 20.
                self.speed_x = -self.speed_x
                reward = HIT_REWARD

        if self.circle_x >= self.bar2_x - 15.:
            if self.circle_y >= self.bar2_y - 15 and self.circle_y <= self.bar2_y + 85:
                self.circle_x = 605.
                self.speed_x = -self.speed_x
